...
2. For each numbered acceptance criterion, and for the stated invariant, write a test asserting exactly what it says, in the most direct form the criterion's own wording supports (unit, integration, or property test — you infer test *shape* from the criterion's wording, never from a codebase you cannot see). **Faithful fakes (ALI-155):** When a criterion names an external-system boundary (GitHub, Linear, Supabase, API calls, etc.), write at least one rejection-path test — exercising not just the success case but a failure the real system enforces (e.g., HTTP 422 for invalid input, 401 for auth, database constraint violation). This test verifies the fake rejects what the real system rejects.
