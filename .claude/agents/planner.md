...
3. **Budget headroom** — average `1 - (budget.consumed / budget.total)` across runs that finished naturally (`stop_reason: cycle-empty`, zero backstop fires — `isCleanRun`). **Three consecutive clean runs is the trigger to raise the budget by 1.** `recommendBudgetChange` checks exactly the last three runs in the window, in order, and cites their `generatedAt` timestamps when it fires.
