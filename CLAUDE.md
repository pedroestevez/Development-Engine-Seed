...
- **Amendment gate:** any change to engine files (`.claude/**`, `CLAUDE.md`, `docs/ENGINE.md`, `.github/**`, `scripts/**`) ends in a PR that **only Pedro merges**. In this seed repo, *every* merge to `main` is the Amendment gate. Enforced by branch protection + CODEOWNERS — do not attempt to merge, approve, or bypass. Read that enforcement honestly (finding S5): branch protection is live and guarantees *a PR exists and nothing bypasses it*, but CODEOWNERS is **staged, not load-bearing** — required approvals are 0 by design (self-approval is forbidden and Pedro is the sole collaborator), so it assigns ownership and blocks nothing on its own. "Only Pedro merges" therefore rests on this rule plus the fact that only he holds merge rights — see `docs/GATES.md` for the full row. `.github/**` and `scripts/**` are named explicitly because the gate's *enforcement logic* lives there (finding F1): a PR that reduces a checker to `process.exit(0)` greens every job, so it is an engine change no matter how small the diff.
