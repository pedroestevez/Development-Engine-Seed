...
1. Invoke `runDispatcher()` with the current engine commit SHA and the real `LinearPort` / `GitHubPort` / `WorktreePort` / `AgentPort` / `Clock` adapters. Nothing upstream of that call is yours to check by hand — the Direction gate (no approved cycle → `stop_reason: no-approved-cycle`) and the status-name drift check (a board missing `Ready`/`Parked`/`Needs Pedro` → `stop_reason: gate-hit`, loud, never silent) both run inside the script, first.
